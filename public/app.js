document.addEventListener("DOMContentLoaded", async () => {
    ...
    const loadQBank = async () => {
      try {
        ...
        console.warn("Offline or fetch fail, loading cache", e);
        try {
    ...
    const renderCategories = () => {
      ...
      list.forEach(cat => {
        ...
        txt.style.textShadow = "0 2px 4px rgba(0,0,0,0.8)";
    ...
    const goWinner = () => {
      ...
      if (winnerTitle && winnerDetails) {
        if (s1 > s2) {
          ...
          winnerDetails.textContent = "أداء رائع! مبروك الفوز.";
        } else if (s2 > s1) {
          winnerTitle.textContent = `الفائز: ${state.team2Name} 🎉`;
          winnerDetails.textContent = "أداء رائع! مبروك الفوز.";
        } else {
          winnerTitle.textContent = "تعادل 🤝";
          winnerDetails.textContent = "يا لها من مباراة قوية!";
        }
      }
    ...
    const openQuestion = (categoryId, points, idx) => {
      if (!QLOOKUP) {
        alert("جار تحميل الأسئلة، يرجى الانتظار...");
        return;
      }
    ...
    const reportQuestion = () => {
      ...
      const reason = prompt("ما هي المشكلة في السؤال؟") || "";
      ...
      saveReports(list);
      alert("شكرًا لك، تم تسجيل الملاحظة.");
    };
    ...
    const applyLifeline = (team, key) => {
      if (!state?.currentQuestionId) {
        alert("يجب فتح سؤال أولاً لاستخدام المساعدة.");
        return;
      }
  
      if (key === "double" && team !== state.currentTurnTeam) {
        alert("يمكن استخدام (الدبل) فقط للفريق الذي عليه الدور.");
        return;
      }
    ...
    const restoreIfAny = () => {
      ...
      if (state.currentQuestionId) {
        ...
        if (q) {
          if (qMeta) qMeta.textContent = `${q.categoryName} • ${q.points}`;
          if (qText) qText.textContent = q.question;
    ...
    if (newGameBtn) {
      newGameBtn.addEventListener("click", () => {
        if (!confirm("هل أنت متأكد من إنهاء اللعبة الحالية؟")) return;
        clearState();
        selected = new Set();
        show(sHome);
      });
